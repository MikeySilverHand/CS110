...
max_countries_three = cases['location'].unique()
# ------------------------------------------------------------


# ------------------------------------------------------------
# All of the countries who dont have any data in active cases
check = cases[cases['location'].isin(max_countries_one) & cases['location'].isin(max_countries_two)]
temp = check.groupby('location')['new_cases'].sum()
max_countries_four = check[(check['new_cases'] != '') & (check['new_cases'].notna())]
# ------------------------------------------------------------
...
cases_filtered = max_countries_four[(max_countries_four['Date'] >= '2021-01-01') & (max_countries_four['Date'] <= '2023-12-31')]
# ------------------------------------------------------------

# ------------------------------------------------------------
# Population with cases/1M in a new dataframe

merged = cases_filtered.merge(countries, left_on='location', right_on='location', how='left')
merged['Cases/1M'] = (merged['new_cases'] / merged['Population']) * 1000000
# ------------------------------------------------------------
...
plt.title('Total Vaccinations by location')
plt.savefig('Total_Vaccinations_by_location.png')
